...
One Hataori Service can coordinate agents whose `activation.workingDirectory` points to one configured workspace. Hataori does not expose a per-client MCP workspace allowlist in version 3.0.2.0. To change the automatic agent workspace, update the absolute `activation.workingDirectory`, run `hataori config check`, and restart the Service.
