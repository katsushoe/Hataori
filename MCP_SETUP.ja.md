...
ServiceがRunningで、MCP JSONに`connected: true`、期待URL、3.0.2.0では`tool_count: 12`があれば合格です。
...
2. **Tool検出:** `tool_count`を確認します。3.0.2.0の合格値は`12`で、`task_start`、`task_get`、`task_list`、`task_heartbeat`、`task_complete`、`task_cancel`、`task_fail`、`task_expire`、`task_history`、`task_relations`、`task_relation_add`、`get_version`です。
